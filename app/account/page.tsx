...
export default function AccountPage() {
  ...
  if (loading) return (
    <main style={{ minHeight: "100dvh", background: "#FFF6E9", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "ui-rounded, system-ui, sans-serif" }}>
      <p style={{ color: "#8a6a5a" }}>Loading your account...</p>
    </main>
  );
    ...
    <main style={{ minHeight: "100dvh", background: "#FFF6E9", fontFamily: "ui-rounded, system-ui, sans-serif", color: "#2B1B12" }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
          ...
          <button onClick={handleSignOut} style={{ background: "none", border: "1.5px solid #ecdccb", borderRadius: 999, padding: "8px 20px", fontSize: 13, fontWeight: 600, color: "#8a6a5a", cursor: "pointer" }}>
            Sign out
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
          {(["orders", "notifications"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              {t === "orders" ? "My Orders" : "Notifications"}
              ...
              return (
                <div key={order.id} style={{ background: "white", borderRadius: 20, padding: 20, border: "1px solid #ecdccb" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                    <div>
                      <p style={{ fontSize: 11, color: "#b09a8a", margin: "0 0 2px", textTransform: "uppercase", letterSpacing: "0.1em" }}>
                        {new Date(order.created_at).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
                      </p>
                      <p style={{ fontSize: 13, color: "#8a6a5a", margin: 0 }}>{order.delivery_day} · {order.borough}</p>
                    </div>
                    <span style={{
                  ...
                  {/* Items */}
                  <div style={{ borderTop: "1px solid #f1e3d3", paddingTop: 12, marginBottom: 12 }}>
                    {order.order_items?.map((item, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#4a3428", marginBottom: 4 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {notifications.length === 0 ? (
              ...
              <div key={n.id} onClick={() => !n.read && markRead(n.id)} style={{
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 14, color: "#2B1B12", margin: "0 0 4px", fontWeight: n.read ? 400 : 700 }}>{n.message}</p>
