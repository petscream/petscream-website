...
export default function AdminPage() {
  ...
  const updateStatus = async (orderId: string, status: string) => {
    ...
    await supabase.from("order_status_history").insert({ order_id: orderId, status, note: "Updated by admin" });
  ...
  const sendNotification = async () => {
    ...
    await supabase.from("notifications").insert({
      user_id: msgUserId,
      order_id: msgOrderId,
      title: msgTitle || "Message from PetsCream",
      message: msgText.trim(),
      type: "info",
    });
    setMsgSent(true);
    setTimeout(() => { setMsgSent(false); setMsgOrderId(null); setMsgUserId(null); setMsgTitle(""); setMsgText(""); }, 2000);
  };

  const filtered = orders.filter(o => {
    const matchStatus = filterStatus === "all" || o.status === filterStatus;
    const matchSearch = !search || o.customer_name?.toLowerCase().includes(search.toLowerCase()) || o.customer_email?.toLowerCase().includes(search.toLowerCase()) || o.order_number?.includes(search);
    return matchStatus && matchSearch;
  });

  if (!authed || loading) return (
    <main style={{ minHeight: "100dvh", background: "#1a1a2e", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: "white", fontFamily: "ui-rounded, system-ui, sans-serif" }}>Loading...</p>
    </main>
  );
    ...
    <main style={{ minHeight: "100dvh", background: "#f8f4ef", fontFamily: "ui-rounded, system-ui, sans-serif", color: "#2B1B12" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
          {[{ key: "all", label: "All", emoji: "📋", count: orders.length }, ...Object.entries(ORDER_STATUS).map(([k, v]) => ({ key: k, label: v.label, emoji: v.emoji, count: orders.filter(o => o.status === k).length }))].map(s => (
            <button key={s.key} onClick={() => setFilterStatus(s.key)} style={{
              ...
              {s.emoji} {s.label}
              <span style={{ background: "#f1e3d3", borderRadius: 999, padding: "1px 7px", fontSize: 11 }}>{s.count}</span>
            </button>
          ))}
        </div>

        <input placeholder="Search by name, email or order number..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {filtered.length === 0 && <p style={{ textAlign: "center", color: "#8a6a5a", padding: "40px 0" }}>No orders found.</p>}
          {filtered.map(order => {
            ...
            return (
              <div key={order.id} style={{ background: "white", borderRadius: 20, padding: 20, border: "1px solid #ecdccb", boxShadow: "0 2px 8px rgba(43,27,18,0.05)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 12 }}>
                  <div>
                    <p style={{ fontWeight: 800, fontSize: 15, margin: "0 0 2px" }}>{order.order_number} — {order.customer_name}</p>
                    <p style={{ fontSize: 12, color: "#8a6a5a", margin: 0 }}>{order.customer_email} · {order.customer_phone}</p>
                    <p style={{ fontSize: 12, color: "#8a6a5a", margin: "2px 0 0" }}>{order.street_address}{order.apt ? ` ${order.apt}` : ""}, {order.borough}</p>
                    <p style={{ fontSize: 12, color: "#8a6a5a", margin: "2px 0 0" }}>🚚 {order.delivery_time_slot}</p>
                    {order.delivery_note && <p style={{ fontSize: 12, color: "#b09a8a", margin: "4px 0 0", fontStyle: "italic" }}>"{order.delivery_note}"</p>}
                  </div>
                  <div style={{ textAlign: "right" }}>
                    ...
                    <select value={order.status} onChange={e => updateStatus(order.id, e.target.value)} style={{
                      background: st.color + "15", color: st.color, border: `1.5px solid ${st.color}`,
                      borderRadius: 999, padding: "5px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer", outline: "none",
                    }}>
                  ...
                  <div>
                    {msgOrderId === order.id ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        <input value={msgTitle} onChange={e => setMsgTitle(e.target.value)} placeholder="Title (optional)" style={{ padding: "8px 12px", border: "1.5px solid #ecdccb", borderRadius: 10, fontSize: 13, outline: "none" }} />
                        <div style={{ display: "flex", gap: 8 }}>
                          <input value={msgText} onChange={e => setMsgText(e.target.value)} placeholder="Message to customer..." style={{ flex: 1, padding: "8px 12px", border: "1.5px solid #ecdccb", borderRadius: 10, fontSize: 13, outline: "none" }} onKeyDown={e => e.key === "Enter" && sendNotification()} />
                          <button onClick={sendNotification} style={{ background: "#2FB7B5", color: "white", border: "none", borderRadius: 10, padding: "8px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>{msgSent ? "Sent ✓" : "Send"}</button>
